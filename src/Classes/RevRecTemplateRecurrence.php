<?php
/**
 * This file is part of the SevenShores/NetSuite library
 * AND originally from the NetSuite PHP Toolkit.
 *
 * New content:
 * @package    ryanwinchester/netsuite-php
 * @copyright  Copyright (c) Ryan Winchester
 * @license    http://www.apache.org/licenses/LICENSE-2.0 Apache-2.0
 * @link       https://github.com/ryanwinchester/netsuite-php
 *
 * Original content:
 * @copyright  Copyright (c) NetSuite Inc.
 * @license    https://raw.githubusercontent.com/ryanwinchester/netsuite-php/master/original/NetSuite%20Application%20Developer%20License%20Agreement.txt
 * @link       http://www.netsuite.com/portal/developers/resources/suitetalk-sample-applications.shtml
 *
 * generated:  2021-01-29 05:23:50 PM UTC
 */

namespace NetSuite\Classes;

class RevRecTemplateRecurrence {
    /**
     * @var RecordRef
     */
    protected RecordRef $incomeaccount;

    /**
     * @var int
     */
    protected int $periodOffset;

    /**
     * @var string
     */
    protected string $recamount;

    static $paramtypesmap = array(
        "incomeaccount" => "RecordRef",
        "periodOffset" => "integer",
        "recamount" => "string",
    );

    /**
     * @param RecordRef $incomeaccount
     * @return RevRecTemplateRecurrence
     */
    public function setIncomeaccount(RecordRef $incomeaccount): RevRecTemplateRecurrence
    {
        $this->incomeaccount = $incomeaccount;
        return $this;
    }

    /**
     * @return RecordRef
     */
    public function getIncomeaccount(): RecordRef
    {
        return $this->incomeaccount;
    }

    /**
     * @param int $periodOffset
     * @return RevRecTemplateRecurrence
     */
    public function setPeriodOffset(int $periodOffset): RevRecTemplateRecurrence
    {
        $this->periodOffset = $periodOffset;
        return $this;
    }

    /**
     * @return int
     */
    public function getPeriodOffset(): int
    {
        return $this->periodOffset;
    }

    /**
     * @param string $recamount
     * @return RevRecTemplateRecurrence
     */
    public function setRecamount(string $recamount): RevRecTemplateRecurrence
    {
        $this->recamount = $recamount;
        return $this;
    }

    /**
     * @return string
     */
    public function getRecamount(): string
    {
        return $this->recamount;
    }

}
